...
function process_form_submission() {
  if (isset($_POST['submit'])) { // Vérifier si le formulaire a été soumis
      ...
      $table_name = $wpdb->prefix . 'commandes'; // Nom complet de la table avec le préfixe de la base de données

      // Récupérer les valeurs des champs du formulaire
      $boisson = sanitize_text_field($_POST['boisson']);
      $nom = sanitize_text_field($_POST['nom']);
      $prenom = sanitize_text_field($_POST['prenom']);
      $email = sanitize_email($_POST['email']);
      $livraison = isset($_POST['livraison']) ? 1 : 0;
      $adresse_livraison = sanitize_text_field($_POST['adresse_livraison']);
      ...
      // Afficher un message de confirmation ou effectuer toute autre action nécessaire
